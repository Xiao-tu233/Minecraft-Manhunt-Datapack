# Parent function: manhunt:get_next
execute store result score #current_alive_id var run data get storage manhunt: alive_runners[0].id
execute if score #alive_runner_found var matches 1 run function manhunt:compass/switch_tracker/get_next/set_target
...
execute unless data storage manhunt: alive_runners[0].sentinel run function manhunt:compass/switch_tracker/get_next/loop