...
- [x] 计时器失效如果所有玩家退出一次之后
- [!] 当导入配置start_countdown被包含时宏函数疑似会报错: 复现失败
- [!] 游戏结束时条件有时无法正确结算: 复现失败
- [x] 考虑更改结算时血量广播的措辞
- [x] 开始游戏 到 倒计时到时间 这段时间的动作栏应该属于常驻显示的范畴 应该由 manhunt:actionbar 函数控制

- [ ] 开始游戏时记录所有Hunter的uid在一个列表 在start_countdown time up的时候在列表中移除被解除效果的Hunter的uid 在任何Hunter上线的时候检查玩家id是否在列表内 如果存在则解除该玩家药水效果 如果列表为空则不在上线时检测
- [ ]  通过给指南针添加Owner自定义标签可以识别指南针拥有者从而在比较靠近的两名玩家切换目标时避免混淆提出切换的玩家(丢出指南针的玩家)
